<script lang="ts">
    ...
    const uniqueId =
        id ?? generateButtonId(`Font${format.charAt(0).toUpperCase() + format.slice(1)}`);
    ...
    <Dropdown bind:isOpen simple triggeredBy="#{uniqueId}">
        {#if format === 'fontFamily'}
            {#each fontFamilies as font}
                <DropdownItem
                    onclick={() => setFontFamily(editor, font.value)}
        ...
        {:else if format === 'fontSize'}
            {#each fontSizes as font}
                <DropdownItem
                    onclick={() => setFontSize(editor, font.value)}
                    style="font-size: {font.value};"
                >
                    {font.name}
                </DropdownItem>
            {/each}
            <DropdownItem onclick={() => removeFontSizeFormatting(editor)}
                >Remove formatting</DropdownItem
            >
        {:else if format === 'textColor'}
            <div class="p-2">
                ...
                <DropdownItem onclick={() => removeTextColorFormatting(editor)} class="text-center"
                    >Remove color</DropdownItem
                >
            </div>
        {/if}
    </Dropdown>
