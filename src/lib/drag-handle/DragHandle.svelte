<script lang="ts">
    ...
    const defaultClass =
        'transition-opacity cursor-grab active:cursor-grabbing bg-gray-300 hover:bg-gray-400 rounded border border-gray-400 w-6 h-6 flex items-center justify-center shadow-sm';
...
<div bind:this={dragElement} class="drag-handle {className}" aria-label={ariaLabel} {...restProps}
    >⋮⋮</div
>
